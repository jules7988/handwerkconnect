...
export default function HomeScreen() {
  ...
  const candidate = candidates[index % candidates.length] as any;

  const like = () => {
    if (currentUser.role === 'firma') createMatch(candidate.id, (currentUser as any).id);
    else createMatch((currentUser as any).id, candidate.id);
  ...
  return (
    <View style={styles.container}>
      ...
      <ItsAMatchModal
        ...
        names={{ azubi: currentUser.role === 'firma' ? candidate.name : (currentUser as any).name, firma: currentUser.role === 'firma' ? (currentUser as any).name : candidate.name }}
      />
    </View>
  );
}
