...
export default function AzubiProfile() {
  ...
  const isValid = useMemo(() => {
    const hasContact = (email?.trim()?.length ?? 0) > 3 || (whatsappLink?.trim()?.length ?? 0) > 10;
    return (
  ...
  const onSave = async () => {
    if (!isValid) return Alert.alert('Fehler', 'Bitte alle Pflichtfelder ausfüllen (Kontakt: E-Mail oder WhatsApp-Link).');
  ...
  return (
    <ScrollView contentContainerStyle={{ padding: 24, gap: 12 }}>
      ...
      <TextInput placeholder="Straße" value={street} onChangeText={setStreet} style={{ borderWidth: 1, borderRadius: 10, padding: 12 }} />
      <TextInput placeholder="Hausnummer" value={houseNo} onChangeText={setHouseNo} style={{ borderWidth: 1, borderRadius: 10, padding: 12 }} />

      <TextInput placeholder="PLZ" value={plz} onChangeText={setPlz} keyboardType="number-pad" style={{ borderWidth: 1, borderRadius: 10, padding: 12 }} />
      <TextInput placeholder="Stadt" value={city} onChangeText={setCity} style={{ borderWidth: 1, borderRadius: 10, padding: 12 }} />
